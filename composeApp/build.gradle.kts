...
val apiBaseUrl: String = run {
    ...
    if (localPropertiesFile.exists()) {
        val props = Properties()
        localPropertiesFile.inputStream().use { props.load(it) }
        props.getProperty("API_BASE_URL", "https://gateway-dev-1b7e.up.railway.app/")
    } else {
        "https://gateway-dev-1b7e.up.railway.app/"
    }
}

val generateJvmAppConfig by tasks.registering {
    val version = appVersionName.get()
    val baseUrl = apiBaseUrl
    val outputFile = layout.projectDirectory.file("src/jvmMain/kotlin/com/coffeepeek/admin/config/JvmAppConfig.kt")
    ...
    doLast {
        ...
        outputFile.asFile.writeText(
            ...
            // Auto-generated by generateJvmAppConfig. Do not edit manually.
...
tasks.matching { it.name == "compileKotlinJvm" }.configureEach {
    dependsOn(generateJvmAppConfig)
}
